...
function FriendsFactory($localstorage, _, locationFactory, SettingsFactory) {
    ...
    function getFriend(id) {
      return _.filter(service.friends, { 'id': parseInt(id) })[0];
    }

    function updateFriend(friend) {
      var oldFriend = _.filter(service.friends, { 'id': parseInt(friend.id) })[0];
      _.assign(oldFriend, _.omit(friend, 'id'));
      service.save();
    }

    function createFriend(friend) {
      var newId = (service.friends.length > 0) ? _.maxBy(service.friends, 'id').id+1 : 1;
      friend.id = newId; 
